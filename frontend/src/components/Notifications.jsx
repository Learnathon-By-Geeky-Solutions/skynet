...
const Notifications = ({ userId }) => {
    ...
    const fetchUnreadCount = async () => {
        try {
            const res = await axios.get(`/api/user/notifications/unread-count?userId=${userId}`);
    ...
    const fetchNotifications = async () => {
        try {
            const res = await axios.get(`/api/user/notifications?userId=${userId}`);
    ...
    const markAsRead = async () => {
        try {
            await axios.put('/api/user/notifications/mark-as-read', { userId });
            setUnreadCount(0);
        } catch (error) {
